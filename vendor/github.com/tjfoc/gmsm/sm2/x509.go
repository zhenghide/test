...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	case *ecdsa.PublicKey:
		...
		case P256Sm2():
			...
			digest = HashMsgZa(signed, pubkey)
			//end modify
			if !Verify(pubkey, digest, ecdsaSig.R, ecdsaSig.S) {
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv interface{}) (cert []byte, err error) {
	...
	*/
	pubkey := &PublicKey{
		Curve: priv.(*PrivateKey).Curve,
		X:     priv.(*PrivateKey).X,
		Y:     priv.(*PrivateKey).Y,
	}
	digest := HashMsgZa(tbsCertContents, pubkey)
...
func (c *Certificate) CreateCRL(rand io.Reader, priv interface{}, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {
	...
	digest := HashMsgZa(tbsCertListContents, pubkey)
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv interface{}) (csr []byte, err error) {
	...
	digest := HashMsgZa(tbsCSRContents, pubkey)
